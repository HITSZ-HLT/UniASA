...
def preprocess_aasd(args):
    print(f'seed: {args.seed}')
    orig_data_path = 'input/orig_datasets/aasd-from_tacl22'

    with open(os.path.join(orig_data_path, f'aasd.cv{args.fold}.train.mrp'), 'r') as f:
        orig_train_data_points = [json.loads(line) for line in f.readlines()]

    with open(os.path.join(orig_data_path, f'aasd.cv{args.fold}.dev.mrp'), 'r') as f:
        orig_dev_data_points = [json.loads(line) for line in f.readlines()]
    
    with open(os.path.join(orig_data_path, f'aasd.cv{args.fold}.test.mrp'), 'r') as f:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_type', type=str, choices=['aasd'], default='aasd')
    ...
    args.ar_type_2_prompt = {
      'detail': 'is a "Detail" type of argument for',
      'support': 'is a "Support" type of argument for',
      'sequence': 'is a "Sequence" type of argument for',
      'additional': 'is an "Additional" type of argument for',
      'attack': 'is an "Attack" type of argument for'
  }
    args.ar_prompt_2_type = {
        'is a "Detail" type of argument for': 'Detail',
        'is a "Support" type of argument for': 'Support',
        'is a "Sequence" type of argument for': 'Sequence',
        'is an "Additional" type of argument for': 'Additional',
