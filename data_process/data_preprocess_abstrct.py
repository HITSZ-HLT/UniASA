...
def post_process_text(s):
    ...
    s = re.sub(r' +', ' ', s).strip()
    s = s.replace('= .', '[my_placeholder_1]')
    s = s.replace('> .', '[my_placeholder_2]')
    s = s.replace('< .', '[my_placeholder_3]')
    s = s.replace('≤ .', '[my_placeholder_4]')
    s = s.replace('≥ .', '[my_placeholder_5]')
    ...
    for data_point in orig_data_points:
        ...
        for edge in data_point['edges']:
            ar_info_dict_list.append({
                ...
                'type': args.ar_type_2_prompt[edge['label'][8:]]
            })
...
def preprocess_abstrct(args):
    print(f'seed: {args.seed}')
    orig_data_path = 'input/orig_datasets/abstrct-from_tacl22'


    with open(os.path.join(orig_data_path, 'abstrct_train.mrp'), 'r') as f:
        orig_train_data_points = [json.loads(line) for line in f.readlines()]

    with open(os.path.join(orig_data_path, 'abstrct_dev.mrp'), 'r') as f:
        orig_dev_data_points = [json.loads(line) for line in f.readlines()]
    
    with open(os.path.join(orig_data_path, 'abstrct_test.mrp'), 'r') as f:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_type', type=str, choices=['abstrct'], default='abstrct')
    ...
    args.ar_type_2_prompt = {
        ...
        'Partial-Attack': 'is a "Partial-Attack" type of argument for',
    }
